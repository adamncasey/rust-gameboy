...
impl Rom {
    pub fn load(path: &str) -> io::Result<Rom> {
        ...
        let mut rom = Rom { game_title: String::from(""), rom_type: 255, rom_contents: vec![] };
        file.read_to_end(&mut rom.rom_contents)?;

        // Copy out game_title
        rom.game_title = String::from(str::from_utf8(&rom.rom_contents[ROM_TITLE_START..ROM_TITLE_END]).unwrap());
